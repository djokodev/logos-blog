...
if command -v flock >/dev/null 2>&1; then
  ...
  if ! flock -n 9; then
    echo "$(iso_now) [WARN] Backup already running, exiting."
    exit 1
  fi
fi
...
echo "$(iso_now) [INFO] Dumping PostgreSQL database..."
...
echo "$(iso_now) [INFO] Uploading backup set to R2 bucket ${R2_BUCKET_NAME}..."
AWS_ACCESS_KEY_ID="${R2_ACCESS_KEY_ID}" \
AWS_SECRET_ACCESS_KEY="${R2_SECRET_ACCESS_KEY}" \
AWS_DEFAULT_REGION="auto" \
"${AWS_BIN}" s3 cp "${backup_dir}/" "s3://${R2_BUCKET_NAME}/${R2_PREFIX}/${timestamp}/" \
  --recursive \
  --endpoint-url "${R2_ENDPOINT}" \
  --only-show-errors

echo "$(iso_now) [INFO] Applying local retention (keep ${BACKUP_RETENTION_WEEKS} backups)..."
if [[ -d "${BACKUP_BASE_DIR}" ]]; then
  ...
  if (( ${#local_dirs[@]} > BACKUP_RETENTION_WEEKS )); then
    ...
    for old_dir in "${local_dirs[@]:0:delete_count}"; do
      ...
      echo "$(iso_now) [INFO] Deleted local backup ${old_dir}"
    done
  fi
fi

echo "$(iso_now) [INFO] Applying R2 retention (keep ${BACKUP_RETENTION_WEEKS} backups)..."
prefixes_raw="$(
  AWS_ACCESS_KEY_ID="${R2_ACCESS_KEY_ID}" \
  AWS_SECRET_ACCESS_KEY="${R2_SECRET_ACCESS_KEY}" \
  AWS_DEFAULT_REGION="auto" \
...
if [[ -n "${prefixes_raw}" ]]; then
  ...
  if (( ${#r2_prefixes[@]} > BACKUP_RETENTION_WEEKS )); then
    ...
    for old_prefix in "${r2_prefixes[@]:0:delete_count}"; do
      ...
      AWS_SECRET_ACCESS_KEY="${R2_SECRET_ACCESS_KEY}" \
      AWS_DEFAULT_REGION="auto" \
      "${AWS_BIN}" s3 rm "s3://${R2_BUCKET_NAME}/${old_prefix}" \
        --recursive \
        --endpoint-url "${R2_ENDPOINT}" \
        --only-show-errors
      echo "$(iso_now) [INFO] Deleted R2 backup ${old_prefix}"
    done
  fi
fi
